...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance p : getInvoice().getPerformances()) {
            ...
            switch (play.getType()) {
                case "tragedy":
                    thisAmount = Constants.FOURTY_THOUSAND;
                    if (p.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                        thisAmount += Constants.ONE_THOUSAND * (p.getAudience() - Constants.THIRTY);
                    }
                    break;
                case "comedy":
            ...
            result.append(String.format("  %s: %s (%s seats)%n", play.getName(),
                    frmt.format(thisAmount / Constants.ONE_HUNDRED), p.getAudience()));
